...
A large portion of my first few weeks was spent learning how  \todo{Complete}
...
\subsection{OIL} 
\label{OIL} 
\todo{Should probably go into appendices}
...
CSS is implemented in Java and user Java Server Faces. A few weeks were spent, in addition to working on SMART, researching and experimenting with JSF and CSS. Ultimately little functionality was achieved.  \todo{More? Find notes on JSF} 
...
These requirements were very broad and difficult to account for. After researching user roles and access privileges in Spring I was satisfied that they could be integrated as an aspect (p. \pageref{AOP}) later in development. This left me to concentrate on building the application to allow for later extension. In order for the application to be used and supported in the future it had to be as maintenance free as possible. Changes to the issuer setup process should not require code refactoring. After working with SMART it was understood that such an approach was feasible. The principle of dependency injection employees a similar idea to the dynamic loading of requests in SMART, although it is \textbf{substantially} more complicated. 
