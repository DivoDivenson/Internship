...
\subsubsection{Software development management tool; "Terra"} 
\label{Terra} 
...
\section{SMART} 
The MasterCard Authorization Rules Tool, internally named SMART, is a platform testing tool used to send mock data to the Retail API Server. SMART is introduced and its function within InControl is explained. A brief overview of the objectives is then presented followed by a more detailed description of SMART.

The first objective is broken down into three sections. This mirrors how the work was done. The second objective is again broken down into three sections. These cover the three main types of maintenance carried out on SMART, whereas in the first objective the work done in each section is a refinement of the previous section.
...
A slight concession was made when handling binary attributes. OIL does not have a binary type, only string and number. Binary attributes are simply strings that can only be "Y" or "N". If an attribute has a "Y" or "N" as default data, it is presumed to be a binary attribute and displayed as a combo-box.
